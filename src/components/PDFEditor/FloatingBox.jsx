...
export default function FloatingBox({ fb, isSel, zoom = 1, rotation = 0, onSelect, onStartDrag, onStartResize, onStartRotate, onUpdate, onCommit, onDelete }) {
  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{ position: "absolute", left: fb.x * zoom, top: fb.y * zoom, minWidth: 180, zIndex: 1000, border: `1px solid ${GOLD}`, borderRadius: 4, background: "#fff", boxShadow: "0 4px 15px rgba(0,0,0,0.2)", boxSizing: "border-box", transform: `translate(-50%, -50%) rotate(${angle}deg)`, transformOrigin: "center center", overflow: "visible" }}>
       <div onMouseDown={onStartRotate} style={{ position: "absolute", top: -14, right: -14, width: 28, height: 28, background: INK, cursor: "crosshair", borderRadius: "50%", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", zIndex: 20 }} title="Drag to Rotate">
          <RotateIcon />
       </div>

       <div onMouseDown={onStartDrag} style={{ background: INK, padding: "4px 8px", cursor: "grab", display: "flex", alignItems: "center", gap: 6, borderRadius: "3px 3px 0 0", userSelect: "none" }}>
          ...
          <button onClick={() => onUpdate({ bgColor: "transparent" })} onMouseDown={stopAll} style={{ fontSize: 9, background: "transparent", color: PARCHMENT, border: `1px solid ${GOLD}`, borderRadius: 2, padding: "1px 4px", cursor: "pointer" }} title="No background">No BG</button>
          <button onClick={() => onDelete()} onMouseDown={stopAll} style={{ marginLeft: "auto", background: LACQUER, color: PARCHMENT, border: `1px solid ${GOLD}`, borderRadius: 3, padding: "1px 6px", cursor: "pointer", fontWeight: "bold", fontSize: 10 }}>X</button>
       </div>
       ...
       <div onMouseDown={onStartResize} style={{ position: "absolute", bottom: -8, right: -8, width: 16, height: 16, background: INK, cursor: "nwse-resize", borderRadius: "50%", border: "2px solid #fff", zIndex: 20 }} />
    </div>
  );
}