...
function Homepage({ onFile, onDropFile, onCreateBlank }) {
  ...
  return (
    <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
      ...
      <header style={{ padding: "12px 20px 14px", textAlign: "center" }}>
        <div style={{ height: 1, background: LACQUER, maxWidth: 920, margin: "0 auto 0", opacity: 0.5 }} />
        <img
          src="/logo.png"
          alt="katanapdf"
          style={{
  ...
  return (
    <footer style={{ background: INK, padding: "26px 20px", textAlign: "center", borderTop: `1px solid rgba(196,150,58,0.3)` }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <a href="mailto:katanapdf@gmail.com?subject=katanapdf%20feedback" style={linkStyle}>Contact</a>
      </div>
...
const FONT_OPTIONS = [
  ...
  { v: "'Brush Script MT', cursive", l: "Brush Script" },
...
function FloatingBox({ fb, isSel, onSelect, onStartDrag, onUpdate, onDelete, onCommit }) {
  ...
  // Resize via dragging the bottom-right handle
  const startResize = (e) => {
    e.preventDefault();
    e.stopPropagation();
    const startX = e.clientX, startY = e.clientY;
    const rect = wrapRef.current.getBoundingClientRect();
    const startW = fb.w || rect.width;
    const startH = fb.h || rect.height;
    setResizing(true);
    const onMove = (ev) => {
  ...
  // Not selected — render text inline like real text on the page (no chrome)
  if (!isSel) {
    return (
      <div onClick={e => { e.stopPropagation(); onSelect(); }}
        style={{
          position: "absolute", left: fb.x, top: fb.y,
          width: fb.w || "auto", minWidth: 20,
  ...
  // Selected — minimal chrome, transparent body, floating toolbar above + hint below
  return (
    <div ref={wrapRef} onClick={e => e.stopPropagation()} style={{
      position: "absolute", left: fb.x, top: fb.y,
      width: fb.w || "auto", minWidth: 80,
      zIndex: 200,
      background: "transparent",
      boxSizing: "border-box",
    }}>
      {/* Toolbar floats ABOVE the text */}
      <div onMouseDown={onStartDrag} style={{
        position: "absolute", bottom: "100%", left: 0,
        marginBottom: 4,
        background: LACQUER, padding: "6px 10px", color: PARCHMENT,
        cursor: "grab", display: "flex", alignItems: "center", gap: 8,
        userSelect: "none", whiteSpace: "nowrap",
        border: `1px solid ${GOLD}`, fontFamily: CINZEL,
        boxShadow: "0 2px 8px rgba(0,0,0,0.2)",
      }}>
        <span style={{ fontWeight: 700, fontSize: 14, cursor: "grab" }}>✥</span>

        <select value={FB_SIZES.includes(fb.fontSize) ? fb.fontSize : 14}
          onChange={e => onUpdate({ fontSize: +e.target.value })}
          onMouseDown={stopAll} onClick={e => e.stopPropagation()}
          style={{ fontSize: 13, background: "#5a0f0f", color: PARCHMENT, border: `1px solid ${GOLD}`, cursor: "pointer", width: 56, padding: "3px 4px", fontFamily: "inherit" }}>
          {FB_SIZES.map(s => <option key={s} value={s}>{s}</option>)}
        </select>

        <button onMouseDown={stopAll} onClick={e => { e.stopPropagation(); onUpdate({ isBold: !fb.isBold }); }}
          style={{ cursor: "pointer", fontWeight: 900, fontSize: 14, padding: "2px 8px", background: fb.isBold ? PARCHMENT : "transparent", color: fb.isBold ? LACQUER : PARCHMENT, border: `1px solid ${GOLD}`, fontFamily: "inherit" }}>B</button>

        <button onMouseDown={stopAll} onClick={e => { e.stopPropagation(); onUpdate({ isItalic: !fb.isItalic }); }}
          style={{ cursor: "pointer", fontStyle: "italic", fontSize: 14, padding: "2px 8px", background: fb.isItalic ? PARCHMENT : "transparent", color: fb.isItalic ? LACQUER : PARCHMENT, border: `1px solid ${GOLD}`, fontFamily: "inherit" }}>I</button>

        <input type="color" value={fb.color || "#000000"} onChange={e => onUpdate({ color: e.target.value })}
          onMouseDown={stopAll}
          style={{ width: 24, height: 22, border: `1px solid ${GOLD}`, padding: 0, background: "none", cursor: "pointer", flexShrink: 0 }} />

        <select value={fb.fontFamily}
          onChange={e => onUpdate({ fontFamily: e.target.value })}
          onMouseDown={stopAll} onClick={e => e.stopPropagation()}
          style={{ fontSize: 13, background: "#5a0f0f", color: PARCHMENT, border: `1px solid ${GOLD}`, cursor: "pointer", padding: "3px 4px", fontFamily: "inherit", maxWidth: 130 }}>
          {FONT_OPTIONS.map(f => <option key={f.v} value={f.v}>{f.l}</option>)}
        </select>

        <button onMouseDown={stopAll} onClick={e => { e.stopPropagation(); onDelete(); }}
          title="Delete"
          style={{ marginLeft: 4, cursor: "pointer", fontWeight: 700, fontSize: 14, padding: "2px 8px", background: "transparent", color: PARCHMENT, border: `1px solid ${GOLD}`, fontFamily: "inherit" }}>✕</button>
      </div>

      {/* Transparent textarea — looks like the rendered text in real time */}
      <textarea ref={taRef} value={fb.text}
        ...
        style={{
          display: "block",
          width: fb.w ? `${fb.w}px` : "100%",
          minWidth: 80,
          height: fb.h ? `${fb.h}px` : "auto",
          minHeight: (fb.fontSize || 14) * 1.4 + 4,
          border: `1px dashed ${LACQUER}`,
          outline: "none",
          resize: "none",
          background: "rgba(245,237,214,0.3)", // very subtle parchment tint so user can find the box
          padding: "2px 4px",
          fontSize: fb.fontSize, fontFamily: fb.fontFamily,
          fontWeight: fb.isBold ? "bold" : "normal",
          fontStyle: fb.isItalic ? "italic" : "normal",
      ...
      <div onMouseDown={startResize}
        style={{
          ...
          background: LACQUER, border: `2px solid ${PARCHMENT}`,
          cursor: "nwse-resize",
        }}
        title="Drag to resize"
      />

      {/* Tab to save · Esc to cancel — floats below */}
      <div style={{
        position: "absolute", top: "100%", left: 0,
        marginTop: 4,
        fontSize: 10, color: LACQUER, fontFamily: CINZEL,
        letterSpacing: 2, textTransform: "uppercase",
        background: PARCHMENT_2, padding: "3px 8px",
